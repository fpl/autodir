...
autodir - A daemon to create dynamically directories for users/groups and
...
centralized database like LDAP, SQL, NIS.

It also has transparent backup features at unmount time and the advantage of
not depending on systemd to work.

B<Autodir> uses a virtual mount point to bind users/groups real paths to a common tree
(e.g., the usual C</home> path is the virtual path generally used for users' home, which
are created elsewhere when needed). The real paths are typically organized in multi-level
subdirs for efficiency. The real path is created automatically when requested, including
the usual skeleton files copying.

The virtual mount point is managed via the kernel B<autofs> module, so every user/group
directory is monted and unmounted dinamically after a certain lapse of time of inactivity.
After unmount, B<autodir> can also optionally run a backup program for the just unmounted
directory. 
=head1 OPTIONS

Autodir accepts a series of common options and a certain number of sub-options for
each of its modules in way similar to that used in mount(8).

=over 4

=item B<-d> I<directory>

Specifies the virtual base directory, i.e. the mount point for the autofs filesystem.
If I<dir> does not exist, it will be created. An absolute directory path is expected
for this option. The default path is generally C</autohome>, but it can change on the basis
of distribution.
...
A comma-separeted list of options are passed to the module. This options
...
Run in the foreground and log all messages to the console. Helpful for debugging purpose
...
option is absent, the backup will start immediately.

=item B<-p> I<number>

Backup process priority. This value is from 1-40, with 1 being highest priority
and 40 being lowest. The default value is 30.
...
a few options can be used to manage multiple aspects of their configuration. 
Those per-module options are passed as list of comma-separated strings
...
The real directory organizaton level. Autohome creates a sub-tree that uses the
first 1,2,3,... initial letters of the usernames to optimize access times in case of
a database of thousands of users. For instance, the C<username> home directory will
be created under C</real/path/use/us/u> if I<level>=3 or under C</real/path> if
...
Do not check if directory to be created really is the home directory of the user.
...
Rename directory to copy all those home dirs with uid mismatch/stale homes.

=back

=head2 Module autogroup.so

This module is used to deal with groups' homes, by creating them when request,
even for multi-host shared accounts, as in the case of NIS, LDAP or SQL-based
ones. This is a helpful module that allows creating per-group common directories
...
Real directory hierarchy organization level, as for I<autohome>. Default level is 2.
...
Do no monitor directory permissions.
...
to a generic namespace. At any access to a bound directory it
...
The real base directory where all sub-dirs namespace will be created.

=item B<level>=I<number>

The level of organization of sub-dirs under the real directory, as in case
of I<autohome> or I<autogroup>. This is a number starting from 0.
Default level is 2.

=item B<owner>=I<uid>

The owner of all directories created. Default is C<nobody>.

=item B<group>=I<gid>

The group of all directoru created. Default is C<nogroup>.
...
This is replaced with the host name.
...
Debian GNU/Linux and its derivatives already installs required scripts
that just need to enabled.
